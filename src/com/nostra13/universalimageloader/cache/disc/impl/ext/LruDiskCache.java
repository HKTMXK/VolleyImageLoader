...
public class LruDiskCache implements DiskCache {
     ...
     * @param cacheMaxSize Max cache size in bytes. <b>0</b> means cache size is
     *            unlimited.
     * @param cacheMaxFileCount Max file count in cache. <b>0</b> means file
     *            count is unlimited.
     * @throws IOException if cache can't be initialized (e.g.
     *             "No space left on device")
     */
    ...
    @Override
    public CacheEntry get(String imageUri) {
        DiskLruCache.Snapshot snapshot = null;
        try {
            snapshot = cache.get(getKey(imageUri));
            return snapshot == null ? null : new CacheEntry(snapshot.getFile(0));
        } catch (IOException e) {
            L.e(e);
            return null;
        } finally {
    ...
    @Override
    public boolean put(String imageUri, InputStream imageStream, IoUtils.CopyListener listener)
            throws IOException {
    ...
    @Override
    public boolean put(String imageUri, CacheEntry entry) {
        if (entry == null || entry.data == null) {
            return false;
        }
        DiskLruCache.Editor editor = null;
        try {
            editor = cache.edit(getKey(imageUri));
        } catch (IOException e) {
            e.printStackTrace();
            return false;
        }
        ...
        if (entry.data instanceof Bitmap) {
            Bitmap bitmap = (Bitmap) entry.data;
            try {
