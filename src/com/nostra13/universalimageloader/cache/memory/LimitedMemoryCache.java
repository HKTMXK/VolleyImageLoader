...
public abstract class LimitedMemoryCache extends BaseMemoryCache {
     ...
     * any time)
     */
    private final List<CacheEntry> hardCache = Collections.synchronizedList(new LinkedList<CacheEntry>());
    ...
    public boolean put(String key, CacheEntry value) {
        ...
        if (valueSize < sizeLimit) {
            while (curCacheSize + valueSize > sizeLimit) {
                CacheEntry removedValue = removeNext();
    ...
    public void remove(String key) {
        ...
        if (value != null) {
            if (hardCache.remove(value)) {
                cacheSize.addAndGet(-value.size());
            }
        }
    }
    ...
    protected abstract int getSize(CacheEntry value);

    protected abstract CacheEntry removeNext();
}
