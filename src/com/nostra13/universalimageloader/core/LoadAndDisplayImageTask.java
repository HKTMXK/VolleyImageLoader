...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
    ...
    public void run() {
        ...
        try {
            ...
            CacheEntry entry = configuration.memoryCache.get(memoryCacheKey);
            if (entry != null) {
                bmp = (Bitmap) entry.data;
            }
            if (bmp == null || bmp.isRecycled()) {
                ...
                if (bmp != null && options.isCacheInMemory()) {
                    ...
                    configuration.memoryCache.put(memoryCacheKey, new CacheEntry(bmp));
                }
            } else {
    ...
    private Bitmap tryLoadBitmap() throws TaskCancelledException {
        ...
        try {
            File imageFile = null;
            CacheEntry cacheEntry = configuration.diskCache.get(uri);
            if (cacheEntry!=null) {
                imageFile = (File) cacheEntry.data;
            }
...
//                    imageFile = configuration.diskCache.get(uri);
                    cacheEntry = configuration.diskCache.get(uri);
                    if (cacheEntry!=null) {
                        imageFile = (File) cacheEntry.data;
                    }
                    if (imageFile != null) {
    ...
    private boolean downloadImage() throws IOException {
        ...
        if (is == null) {
            ...
            try {
                return configuration.diskCache.put(uri, is, this);
            } finally {
    ...
    private boolean resizeAndSaveImage(int maxWidth, int maxHeight) throws IOException {
        ...
        CacheEntry targetEntry = configuration.diskCache.get(uri);
        if (targetEntry!=null) {
            targetFile = (File) targetEntry.data;
        }
        if (targetFile != null && targetFile.exists()) {
            ...
            if (bmp != null) {
                saved = configuration.diskCache.put(uri, new CacheEntry(bmp));
                bmp.recycle();
            }
        }
        return saved;
